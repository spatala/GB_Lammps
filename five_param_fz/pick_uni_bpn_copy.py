...
def pick_uni_bpn(num, sigma_val, lat_type, bound=10, plot_sw=False):
    ...
    for i in range(len(pkl_content[str(sigma_val)]['N'])):
        ...
        bpn, _ = csl_area_sort(bpn_sphr, l_rcsl_go, mt_rcsl_go)
        ...
        bpn_grid_fz_int = GBim.int_finder(bpn_grid_fz, tol=1e-06, order='rows')
        if plot_sw == True:
            plot_2d(bpn_grid, gs)
        ...
        pub_out.append([bpn_grid_fz, bpn_grid_fz_int, symm_grp_ax, bp_symm_grp])
    print pub_out
    return pub_out
...
def csl_area_sort(bpn, l_rcsl_go, mt_cslr_go):
    ...
    return bpn_sort, ind_d_inv_sort
...
def bpn_in_grid(bpn, grid, n, l_rcsl_go, mt_rcsl_go, tol=1e-06):
    ...
    bpn_grid_sphr = mf.cube2sphr(bpn_grid)
...
def grid(gs):
    ...
    grid_lines_sphr =  GBt.unique_rows_tol(mf.cube2sphr(grid_lines), tol=1e-06)
