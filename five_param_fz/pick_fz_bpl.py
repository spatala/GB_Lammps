...
def pick_fz_bpl(bp_norms_go1, bp_symm_grp, symm_grp_ax, x_tol=1e-04):
    ...
    *  The boundary plane vectors are rotated for steregraphic projection along z_axis (symm_grp_axes[:, 2].
    ...
    ### Rotating the points
    # t1_vecs = np.dot(rot_mat, symm_bpn_go1.transpose((1, 2, 0))).transpose((2, 1, 0))
...
#
# bsg = 'D_4h'
# bng = np.array([[0.4662524, -0.83925433, -0.27975144], [0.3333333, -0.3333333, -0.3333333]])
# xt = 1e-04
# sga = np.array([[0, 0, 1], [-0.31622777, -0.9486833, 0], [0.9486833, -0.31622777, 0]])
