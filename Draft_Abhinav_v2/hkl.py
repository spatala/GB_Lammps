...
def hkl(mesh_size, sigma_val, lat_type):
    ...
    # mil_ind_csl = np.vstack((mil_ind_csl0, mil_ind_csl1, mil_ind_csl2, mil_ind_csl3))
                         #     [0, 1, 0], [1, 1, 0], [1, 2, 0], [1, 4, 0],
                         #     [1, 2**3, 0], [1, 2**4, 0],
                         # [1, 4, 2**0], [1, 4, 2**1], [1, 4, 2**2], [1, 4, 2**3], [1, 4, 2**4], [1, 4, 2**5], [1, 4, 2**6], [1, 4, 2**7], [0, 0, 1]    


    # mil_ind_csl = (np.array([[1, 0, 0], [1, 0, 2**0], [1, 0, 2**1], [1, 0, 2**2], [1, 0, 2**3], [1, 0, 2**4], [1, 0, 2**5], 
    #                          [1, 1, 0], [1, 1, 2**0], [1, 1, 2**1], [1, 1, 2**2], [1, 1, 2**3], [1, 1, 2**4], [1, 1, 2**5], 
    #                          [2, 3, 0], [2, 3, 2**0], [2, 3, 2**1], [2, 3, 2**2], [2, 3, 2**3], [2, 3, 2**4], 
    #                          [1, 2, 0], [1, 2, 2**0], [1, 2, 2**1], [1, 2, 2**2], [1, 2, 2**3], [1, 2, 2**4], 
    #                          [1, 4, 0], [1, 4, 2**0], [1, 4, 2**1], [1, 4, 2**2], [1, 4, 2**3], [1, 4, 2**4], 
    #                          [0, 0, 1],
    #                      #     [0, 1, 0], [1, 1, 0], [1, 2, 0], [1, 4, 0],
    #                      #     [1, 2**3, 0], [1, 2**4, 0],
    #                      # [1, 4, 2**0], [1, 4, 2**1], [1, 4, 2**2], [1, 4, 2**3], [1, 4, 2**4], [1, 4, 2**5], [1, 4, 2**6], [1, 4, 2**7], [0, 0, 1]    
